...
use Koverae\KoveraeBilling\Exceptions\UsageDenied;
use Koverae\KoveraeBilling\Services\Period;
use Koverae\KoveraeBilling\Traits\BelongsToPlan;
use Koverae\KoveraeBilling\Traits\HasFeatures;
use Koverae\KoveraeBilling\Traits\HasGracePeriod;
use Koverae\KoveraeBilling\Traits\HasGracePeriodUsage;
use Koverae\KoveraeBilling\Traits\HasPricing;
use Koverae\KoveraeBilling\Traits\HasSubscriptionPeriodUsage;
use Koverae\KoveraeBilling\Traits\HasTrialPeriodUsage;
use Koverae\KoveraeBilling\Traits\HasSchedules;
use Carbon\Carbon;
// use DB;
...
class PlanSubscription extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('koverae-billing.tables.plan_subscriptions'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                ...
                'alpha_dash',
                'max:150',
                Rule::unique(config('koverae-billing.tables.plan_subscriptions'))->where(function ($query) {
    ...
    {
        return $this->hasMany(config('koverae-billing.models.plan_subscription_feature'), 'plan_subscription_id', 'id');
    }
    ...
    public function usage(): \Illuminate\Database\Eloquent\Relations\HasManyThrough
    {
        return $this->hasManyThrough(
            config('koverae-billing.models.plan_subscription_usage'),
            config('koverae-billing.models.plan_subscription_feature'),
    ...
    {
        if (!$ignoreFallback && config('koverae-billing.fallback_plan_tag')) { // Do not cancel if a fallback plan is set
            $plan = Plan::getByTag(config('koverae-billing.fallback_plan_tag'));
            if (!$plan) {
                throw new UnexpectedValueException('Fallback plan ' . config('koverae-billing.fallback_plan_tag') . ' does not exist.');
            }
     ...
     * @param int $uses
     *
     * @return \Koverae\KoveraeBilling\Models\PlanSubscriptionUsage|null
     */
