...
namespace Koverae\KoveraeBilling\Jobs;
...
class SubscriptionSchedulePaymentJob implements ShouldQueue
{
    ...
    public function __construct($planSubscriptionScheduleId)
    {
        ...
        $paymentMethod = config('koverae-billing.services.payment_methods.' . $this->planSubscriptionSchedule->subscription->payment_method);

        // Check if service exists in config file
        if (empty($paymentMethod)) {
            throw new ExceptionInvalidArgumentException('Selected payment method does not exist', 401);
        }
